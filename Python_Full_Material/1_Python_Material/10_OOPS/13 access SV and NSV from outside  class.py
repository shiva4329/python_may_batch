...
print(s1.y)#if we execute these 2 stmts without executing s1.display()method
         #we get error bcoz, values wont be allocated to x and y,so call s1.dispaly() first.
         #but without calling a method, I want NSV to get initialized ,then go for constructors.
         #To overcome this, allocate values for NSV during object creation only using constructor.
